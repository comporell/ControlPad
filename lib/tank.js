...
var headRotate2 = function headRotate2(angle){
    angle = sg90minRange + (angle/180)*(sg90maxRange-sg90minRange);
    b.analogWrite(pwm02, angle, 20);
...
var odata = function odata(dataPort,dataValue,dataDuration){

    //b.digitalWrite(datapins[dataPort],b.HIGH);
    
    switch(dataDuration){
        case "-1":
            b.digitalWrite(datapins[dataPort],b.LOW);
            break;
        case "0":
            b.digitalRead(datapins[dataPort], switchState);
            break;
        default:
            b.digitalWrite(datapins[dataPort],b.HIGH);
            clearTimeout(tf);
            tf=setTimeout(function(){b.digitalWrite(datapins[dataPort],b.LOW);},dataDuration);
            break;
        }
...
var exitProgram = function exitProgram(){
    ...
    for(var x in allpins){
        for(var y in allpins[x]){
            b.digitalWrite(allpins[x][y],b.LOW);
        }
    }
    return true;
...
module.exports.exitProgram = exitProgram;
